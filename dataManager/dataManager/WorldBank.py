...
def dict2htmltable(data):
    html = '<thead class="sticky top-0 text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">' + '<tr>' + f'<th colspan="{len(data[0].keys())}" class="heads py-3 px-6 font-medium text-white text-center"> </th>' + '</tr>' + '<tr>' + ''.join('<th scope="col" class="py-3 px-6 text-center">' + x + '</th>' for x in data[0].keys()) + '</tr>' + '<tbody>'
